...
class Filesystem:
    ...
    def __init__(self, file: str, sector_size: int = 512, offset: int = 0, size: int = 0):
        ...
        # Read the first(yes, 1, not zero) Cluster,
        # it should contain the root DirectoryEntry list
    ...
    def _cluster_id_offset(self, ID: int):
        ...
        return (
            (ID - 1) * self.sb.cluster_size + SuperBlock.SUPERBLOCK_SIZE + self.fat_size
        )
    ...
    def _calc_fat_size(partition_size: int, cluster_size: int):
        ...
        size = (
            FATX16 if number_of_clusters <= 0xFFF5 else FATX32
        )  # deciding how big a fat entry is in bytes
